    ...
    try:
        ...
        for index, row in enumerate(rows, start=2):
            try:
                ...
                db.table("meters").update({"site_id":site_id,"last_seen_period":row["period_start"],"current_tariff_code":row["tariff"]}).eq("id",meter_id).execute()
        ...
        missing = []
        if billing_period:
            expected = db.table("meters").select("id,tracking_code,meter_number,sites(name)").eq("organization_id",organization_id).eq("status","active").eq("expected_monthly",True).execute().data
