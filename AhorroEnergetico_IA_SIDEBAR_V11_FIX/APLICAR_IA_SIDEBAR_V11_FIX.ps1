...
Write-Host " AHORRO ENERGETICO - MODULO IA EN SIDEBAR V11 FIX" -ForegroundColor Cyan
Write-Host "======================================================" -ForegroundColor Cyan
Write-Host ""
...
$backup=Join-Path $front "backup_ia_sidebar_v11_fix_$stamp"
...
# 4) Boton IA en sidebar.
if($page -notmatch '>IA</span>'){
  $pattern='(?s)(<button className=\{tab==="map"\?"active":""\} onClick=\{\(\)=>setTab\("map"\)\}>.*?<span>Medidores</span></button>)'
  $rx = New-Object System.Text.RegularExpressions.Regex($pattern,[System.Text.RegularExpressions.RegexOptions]::Singleline)
  if($rx.IsMatch($page)){
    $page=$rx.Replace($page,'$1<button className={tab==="ai"?"active":""} onClick={()=>setTab("ai")}><i>✦</i><span>IA</span></button>',1)
  }else{
    $navClose='</nav>'
...
if(($check -match 'tab==="ai"') -and ($check -match 'Asistente energético')){
  Write-Host ""
  Write-Host "======================================================" -ForegroundColor Cyan
  Write-Host " MODULO IA V11 FIX APLICADO" -ForegroundColor Cyan
  Write-Host "======================================================" -ForegroundColor Cyan
  Write-Host ""
