...
def invoices(organization_id:str,meter_id:str|None=None,limit:int=Query(100,ge=1,le=5000),user:CurrentUser=Depends(current_user)):
    require_org(user.id,organization_id)
    # PostgREST puede limitar cada respuesta aunque el cliente solicite más filas.
    # Se pagina en el servidor para que los gráficos históricos no queden truncados.
    ...
    while len(rows)<limit:
        ...
        q=admin_db().table("invoices").select(select).eq("organization_id",organization_id)
        if meter_id:q=q.eq("meter_id",meter_id)
        page=q.order("period_start",desc=True).order("id",desc=True).range(len(rows),len(rows)+size-1).execute().data
        rows.extend(page)
