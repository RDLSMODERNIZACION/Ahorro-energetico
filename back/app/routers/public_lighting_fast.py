...
def _month_key(value) -> str:
    return str(value)[:7] if value is not None else ""
...
def _measurement_profile(history: list[dict]) -> dict:
    states = [_reading_is_coherent(x) for x in history]
    verified = [x for x in states if x is not None]
    coherent = sum(1 for x in verified if x is True)
    incoherent = sum(1 for x in verified if x is False)
...
def _paged(query_builder, page_size: int = 1000):
    rows = []
    offset = 0
    while True:
...
@router.get("/organizations/{organization_id}/public-lighting/analysis-fast")
def public_lighting_analysis_fast(
    organization_id: str,
    ...
    ap_meters = (
        ...
        .select("id,linked_meter_id,supply_number,supply_contract,meter_number,address,tariff_code,validation_status")
    ...
    if linked_ids:
        general_meters = (
            db.table("meters")
            .select("id,meter_number,supply_number,service_name,current_tariff_code")
            .in_("id", linked_ids)
            .execute().data or []
        )
    ...
    invoices = []
    if linked_ids:
        invoices = _paged(lambda offset, size: (
            db.table("invoices")
            .select("id,meter_id,invoice_number,billing_period,period_start,current_tariff_code,total_amount,invoice_measurements(active_energy_kwh)")
            .eq("organization_id", organization_id)
    ...
    for ap in ap_meters:
        ...
        prior = [
            x for x in history
            if _month_key(x.get("billing_period") or x.get("period_start"))
            and _month_index(_month_key(x.get("billing_period") or x.get("period_start"))) < selected_index
        ]
        prior12 = prior[-12:]
        ...
        current_amount = _num((current or {}).get("total_amount"))
        ...
        if current:
            ...
            history_values = [
                _invoice_kwh(x) for x in history
                if _month_key(x.get("billing_period") or x.get("period_start"))
                and _month_index(_month_key(x.get("billing_period") or x.get("period_start"))) <= selected_index
            ]
            level, reasons, change_pct, constant = _analysis_status(
                current_kwh, previous_kwh, avg12, tariff_code, validation_status, history_values
            )
        else:
            level = "missing"
