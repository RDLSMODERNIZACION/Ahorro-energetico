    ...
    try:
        ...
        for index, row in enumerate(rows, start=2):
            try:
                ...
                meter_id = meter_result.data[0]["id"] if meter_result.data else db.table("meters").insert({"organization_id":organization_id,"site_id":site_id,"meter_number":row["meter_number"],"current_tariff_code":row["tariff"],"voltage_level":row["voltage"],"contracted_kw_peak":str(row["contracted"])}).execute().data[0]["id"]
        ...
        db.table("import_batches").update({"status":status,"total_rows":len(rows),"imported_rows":imported,"rejected_rows":rejected,"errors":errors,"completed_at":datetime.utcnow().isoformat()}).eq("id",batch["id"]).execute()
        return {"duplicate":False,"batch_id":batch["id"],"total":len(rows),"imported":imported,"rejected":rejected,"errors":errors[:20]}
    except Exception as exc:
