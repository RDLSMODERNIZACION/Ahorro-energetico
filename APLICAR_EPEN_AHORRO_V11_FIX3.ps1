...
$backup=Join-Path $Root "backup_v11_fix3_$stamp"
...
# 1) Agregar available/reason al tipo si faltan
if($text -match 'type AdvancedTariffHistoryPoint=\{'){
  if($text -notmatch 'available\?:boolean;'){
    $text=[regex]::Replace(
      ...
      ('$1'+[Environment]::NewLine+'  available?:boolean;'+[Environment]::NewLine+'  reason?:string|null;'),
      1
    )
  }
}

# 2) Unificar el cálculo de ahorro
...
'@
  ...
  if($new -eq $text){throw "No pude ubicar el bloque tariffSaving."}
  $text=$new
}

# 3) Reconstruir TariffSavingTrend
...
# 4) Reemplazar advancedRows de forma robusta
$advPattern='const\s+advancedRows=\(advancedTariffHistory\?\.points\|\|\[\]\)\.map\(x=>\(\{[\s\S]*?\}\)\);'
$advReplacement=@'
...
# 5) Reemplazar legacyRows COMPLETO, evitando -replace problemático
$legacyPattern='const\s+legacyRows=tariffSavings[\s\S]*?\.map\(x=>\(\{[\s\S]*?\}\)\);'
$legacyReplacement=@'
...
'@
$text=[regex]::Replace($text,$legacyPattern,$legacyReplacement,1)
...
Write-Host ""
Write-Host "OK - V11 FIX3 aplicada." -ForegroundColor Green
Write-Host "Se corrigió el error de PowerShell -replace." -ForegroundColor Yellow
