...
<article><span>Consumo registrado</span><strong>{number.format(kwh)} <i>kWh</i></strong><small>{activeMeters.length} activos · {lifecycleMeters.length} en seguimiento</small></article><article className="green"><span>Ahorro anual potencial</span><strong>{money.format(tariffAnnual)}</strong><small>{money.format(tariffAnnual/12)} por mes · IVA incluido</small></article><article className={missing.length?"warning-card":""}><span>Facturas faltantes</span><strong>{missing.length}</strong><small>{missing.length?"Requieren revisión":"Seguimiento al día"}</small></article></div>
<section className="panel executive-savings"><Title title="Desglose del ahorro potencial" sub="Cálculo actualizado con potencia usada, cuadro tarifario y 30% de IVA"/><div className="dashboard-savings-grid"><article className="power"><span>Potencia contratada</span><strong>{money.format(powerAnnual)}</strong><small>{money.format(powerAnnual/12)} mensual</small><p>Contratada menos máxima registrada, sin margen.</p></article><article className="reactive"><span>Factor de potencia</span><strong>{money.format(reactiveAnnual)}</strong><small>{money.format(reactiveAnnual/12)} mensual</small><p>Recargos COS que podrían evitarse.</p></article><article className="rate"><span>Cambio tarifario</span><strong>{money.format(rateAnnual)}</strong><small>{money.format(rateAnnual/12)} mensual</small><p>{(tariffSavings.length||assessments.length)?"Diferencia contra la categoría que corresponde.":"Pendiente: el backend no devolvió el análisis tarifario."}</p></article><article className="saving-total"><span>Ahorro total propuesto</span><strong>{money.format(tariffAnnual)}</strong><small>{money.format(tariffAnnual/12)} mensual</small><p>Proyección anual con IVA incluido.</p></article></div></section><div className="dashboard-status"><section className="panel"><Title title="Estado del análisis" sub={`${assessments.length||latestInvoiceByMeter.length} suministros evaluados`}/><div className="status-summary"><article><b>{reviewCount}</b><span>requieren revisión</span></article>
<article><b>{(tariffSavings.length||assessments.filter(x=>x.status==="change_candidate").length)}</b><span>posibles cambios tarifarios</span></article>
<article><b>{powerSavingCount}</b><span>con potencia sobrante</span></article>
<article><b>{reactiveSavingCount}</b><span>con recargo reactivo</span></article></div><button className="dashboard-link" onClick={()=>setTab("framing")}>Ver diagnóstico completo →</button></section>
<aside className="panel findings"><Title title="Alertas mensuales" sub="Medidores que no aparecieron en la carga"/>{missing.length?missing.slice(0,6).map(x=><div className="finding" key={x.id}><i>!</i><div><b>{x.meters?.tracking_code||"Sin ID"}</b><p>{x.message}</p></div></div>):<div className="empty">No hay facturas faltantes.</div>}</aside>
</div><MeterLifecyclePanel meters={lifecycleMeters} latestPeriod={periods[0]||""} onStatus={updateMeterStatus}/></>}
...
{missingPeriodMeters.length>0&&<section className="panel missing-invoice-panel"><Title title={`Faltan ${missingPeriodMeters.length} facturas de ${controlPeriod}`} sub="Estos medidores no aparecen en el archivo del período seleccionado"/><div className="missing-meter-grid">{missingPeriodMeters.map(m=><article key={m.id}><i>!</i><div><b>Medidor {m.meter_number||"S/D"}</b><span>{m.service_name||m.sites?.name||"Servicio sin nombre"}</span><small>{m.tracking_code} · Suministro {m.supply_number||"S/D"}</small></div></article>)}</div></section>}<section className="panel"><Title title="Administrador mensual y anual" sub={`${filteredInvoices.length} de ${meters.length} facturas recibidas para ${controlPeriod}`}/><div className="filters"><label>Año<select value={yearFilter} onChange={e=>setYearFilter(e.target.value)}>{years.map(y=><option key={y}>{y}</option>)}</select></label><label>Mes<select value={monthFilter} onChange={e=>setMonthFilter(e.target.value)}>{Array.from({length:12},(_,i)=>String(i+1).padStart(2,"0")).map(m=><option key={m} value={m}>{new Date(2026,Number(m)-1,1).toLocaleString("es-AR",{month:"long"})}</option>)}</select></label><label className="search-filter">Buscar<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Medidor, suministro, servicio o factura"/></label><button onClick={()=>setSearch("")}>Limpiar búsqueda</button></div><div className="invoice-unified-scroll"><InvoiceTable invoices={filteredInvoices} assessments={assessments} tariffSavings={tariffSavings} pendingMeters={visibleMissingPeriodMeters} period={controlPeriod} onSelect={openMeter}/></div></section>{selectedInvoice&&<InvoiceAnalysisPanel invoice={selectedInvoice} history={invoices.filter(x=>x.meter_id===selectedInvoice.meter_id)} tariffSavings={tariffSavings} onClose={()=>setSelectedInvoice(null)}/>}</>}
  {tab==="framing"&&<><div className="savings-summary"><article><span>Potencia contratada</span><b>{money.format(powerAnnual)}</b><small>ahorro anual</small></article>
