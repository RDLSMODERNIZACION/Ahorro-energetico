...
type Opportunity = {
  ...
  status: string;
  meters?: { meter_number: string; sites?: { name: string } };
};
type TariffAssessment = {
  ...
  status: "correct" | "power_review" | "change_candidate" | "provisional";
  reasons: string[];
  ...
  power_factor?: number;
  power_excess_kw?: number;
  power_unit_price?: number;
  power_saving_source?: "invoice_excess" | "contracted_reduction";
  power_monthly_saving: number;
  power_annual_saving: number;
  reactive_monthly_saving: number;
  reactive_annual_saving: number;
...
type TariffSavingResponse = {
  candidates: TariffSaving[];
  candidate_count: number;
...
function dashboardPowerContract(i: Invoice) {
  const line = Math.max(
    ...
    ...(i.invoice_lines || [])
      .filter((x) =>
        ["DEP", "DEM"].includes(String(x.concept_code || "").toUpperCase()),
      )
      .map((x) => Number(x.quantity || 0)),
  );
  return Number(
    i.contracted_kw_peak || i.meters?.contracted_kw_peak || line || 0,
  );
}
...
function buildDashboardPowerCurve(invoices: Invoice[], period: string) {
  ...
  const meters = meterIds
    .map((meterId) => {
      const history = invoices
        .filter((i) => i.meter_id === meterId && dashboardPowerDemand(i) > 0)
        .sort((a, b) =>
          String(a.billing_period || a.period_start).localeCompare(
            String(b.billing_period || b.period_start),
          ),
        );
      const latestContract = [...history]
        .reverse()
        .find((i) => dashboardPowerContract(i) > 0);
      const latestRate = [...history]
        .reverse()
        .find((i) => dashboardPowerRate(i) > 0);
      const currentKw = latestContract
        ? dashboardPowerContract(latestContract)
        : 0;
      const tariffCode =
      ...
      return {
        ...
        rate,
        rows,
        monthlySaving: Number(selected?.saving || 0),
        annualSaving: rows.reduce((s, r) => s + r.saving, 0),
      };
    })
  ...
  return {
    ...
    opportunityMeterIds: new Set(
      meters.filter((x) => x.monthlySaving > 0).map((x) => x.meterId),
    ),
  };
}

function renderAiRichText(
  text: string,
  ...
  const isReference = (value: string) => {
    ...
    return (
      /(?:medidor|suministro|id)\s*[:#-]?\s*[A-Za-z0-9-]{3,}/i.test(v) ||
      /^[0-9]{5,12}$/.test(v)
    );
  };
  ...
  return normalized
    ...
    .map((raw, index) => {
      ...
      const clickable =
        Boolean(onOpenReference) &&
        /(medidor|suministro)\s*[:#-]?\s*[A-Za-z0-9-]{3,}/i.test(line);

      if (numbered) {
        return (
          <div
            className={`ai-rich-item${clickable ? " clickable" : ""}`}
            key={`n-${index}`}
            onClick={() => clickable && clickFromLine(numbered[2])}
          >
      ...
      if (bullet) {
        return (
          <div
            className={`ai-rich-item${clickable ? " clickable" : ""}`}
            key={`u-${index}`}
            onClick={() => clickable && clickFromLine(bullet[1])}
          >
      ...
      if (line.startsWith("## ")) {
        return (
          <h4 className="ai-rich-heading" key={`h-${index}`}>
            {line.slice(3)}
          </h4>
        );
      }
      return (
        <p
          className={`ai-rich-paragraph${clickable ? " clickable" : ""}`}
          key={`p-${index}`}
          onClick={() => clickable && clickFromLine(line)}
        >
...
export default function Home() {
  ...
  const [epenOptimization, setEpenOptimization] = useState<
    EpenOptimizationMeter[]
  >([]);
  const [advancedTariffSummary, setAdvancedTariffSummary] =
    useState<AdvancedTariffSummary | null>(null);
  ...
  const [organization, setOrganization] = useState<Organization | null>(null),
    ...
    [missing, setMissing] = useState<Missing[]>([]),
  ...
  const [tab, setTab] = useState<
      "dashboard" | "invoices" | "framing" | "tariffs" | "map" | "ai"
    >("dashboard"),
    [busy, setBusy] = useState(false),
    [toast, setToast] = useState(""),
    [selectedMeter, setSelectedMeter] = useState(""),
    [selectedInvoice, setSelectedInvoice] = useState<Invoice | null>(null),
    [yearFilter, setYearFilter] = useState("all"),
    [monthFilter, setMonthFilter] = useState("all"),
    [search, setSearch] = useState(""),
    [framingFilter, setFramingFilter] = useState("all"),
    [mapSearch, setMapSearch] = useState("");
  const [aiQuery, setAiQuery] = useState(""),
    [aiAnswer, setAiAnswer] = useState(
      "Seleccioná una consulta sugerida o escribí qué querés analizar.",
    ),
    ...
    async (s: Session, org?: string) => {
      try {
        ...
        const [m, i, o] = await Promise.all([
          api<Meter[]>(`/api/organizations/${target}/meters`, s),
          api<Invoice[]>(`/api/organizations/${target}/invoices?limit=5000`, s),
          api<Opportunity[]>(
            `/api/organizations/${target}/opportunities`,
            s,
          ).catch(() => []),
          loadChangeControls(target),
        ]);
        setMeters(m);
        setInvoices(i);
        setOpportunities(o);
        setSelectedMeter((current) => current || m[0]?.id || "");
        if (!invoiceFiltersInitialized.current && i.length) {
          const latest = [
            ...new Set(
              i.map((x) => (x.billing_period || x.period_start).slice(0, 7)),
            ),
          ]
            .sort()
            .reverse()[0];
          if (latest) {
            setYearFilter(latest.slice(0, 4));
            setMonthFilter(latest.slice(5, 7));
            invoiceFiltersInitialized.current = true;
          }
        }
        const [miss, frames, tariffResult] = await Promise.all([
          api<Missing[]>(
            `/api/organizations/${target}/missing-invoices`,
            s,
          ).catch(() => []),
          api<TariffAssessment[]>(
            `/api/organizations/${target}/tariff-assessments?v=14`,
            s,
          ).catch(() => []),
          api<TariffSavingResponse>(
            `/api/organizations/${target}/tariff-savings?v=1`,
            s,
          ).catch((e) => {
            setToast(
              `No se pudo cargar el ahorro tarifario: ${e instanceof Error ? e.message : "Error desconocido"}`,
            );
            return null;
          }),
        ]);
        ...
        const epen = await api<EpenOptimizationResponse>(
          `/api/organizations/${target}/epen-optimization?v=2`,
          s,
        ).catch(() => null);
        setEpenOptimization(epen?.meters || []);
      } catch (e) {
        setToast(
  ...
  async function analyze() {
    ...
    setBusy(true);
    try {
      const r = await api<{ opportunities_created: number }>(
        `/api/organizations/${orgId}/analysis/run`,
        session,
        { method: "POST" },
      );
      setToast(`${r.opportunities_created} oportunidades detectadas`);
      await load(session, orgId);
    } catch (e) {
      setToast(e instanceof Error ? e.message : "No se pudo analizar");
    } finally {
      setBusy(false);
    }
  }
  async function runAiQuery(text?: string) {
    ...
    if (!question) {
      setAiAnswer("Escribí una consulta.");
      return;
    }
    if (!session || !orgId) {
      setAiAnswer("No hay sesión u organización activa.");
      return;
    }
    setAiBusy(true);
    try {
      ...
      }>("/api/ai/query", session, {
        method: "POST",
        body: JSON.stringify({ organization_id: orgId, question }),
      });
      setAiAnswer(result.answer);
    } catch (error) {
      setAiAnswer(
        error instanceof Error ? error.message : "No se pudo consultar la IA",
      );
    } finally {
      setAiBusy(false);
    }
  ...
  function openAiReference(reference: string) {
    ...
    if (latest) {
      ...
      setToast(
        `El medidor ${meter.meter_number || reference} no tiene factura para abrir`,
      );
      setTimeout(() => setToast(""), 3500);
    }
  }
  async function updateMeterStatus(
    ...
    setBusy(true);
    try {
      await api(`/api/meters/${meterId}/billing-status`, session, {
        method: "PUT",
        body: JSON.stringify({ status }),
      });
      setToast(
        status === "active"
          ? "El medidor vuelve al seguimiento mensual"
          : status === "removed"
            ? "Baja confirmada: ya no se esperarán facturas"
            : "Medidor marcado como posible baja",
      );
      await load(session, orgId);
    } catch (e) {
      setToast(
        e instanceof Error ? e.message : "No se pudo actualizar el medidor",
      );
    } finally {
      setBusy(false);
  ...
  const total = invoices.reduce((s, x) => s + Number(x.total_amount || 0), 0),
    kwh = invoices.reduce(
      (s, x) =>
        s +
        (x.invoice_measurements || []).reduce(
          (a, m) => a + Number(m.active_energy_kwh || 0),
          0,
        ),
      0,
    ),
  ...
  const activeMeters = meters.filter(
    (m) => (m.status || "active") !== "removed",
  );
  const lifecycleMeters = meters.filter(
    (m) => m.status === "inactive" || m.status === "removed",
  );
  const presentMeterIds = new Set(
    invoices
      .filter((i) => invoiceMonth(i) === controlPeriod)
      .map((i) => i.meter_id),
  );
  const missingPeriodMeters = activeMeters.filter(
    (m) => !presentMeterIds.has(m.id),
  );
  const visibleMissingPeriodMeters = missingPeriodMeters.filter((m) => {
  ...
  const visibleAssessments = assessments.filter(
    (x) => framingFilter === "all" || x.status === framingFilter,
  );
  const latestInvoiceByMeter = [...invoices]
    .sort((a, b) => invoiceMonth(b).localeCompare(invoiceMonth(a)))
    .filter(
      (invoice, index, list) =>
  ...
  const reviewCount = assessments.length
    ...
    : latestInvoiceByMeter.filter(
        (i) => invoicePowerSaving(i).amount > 0 || invoiceReactiveSaving(i) > 0,
      ).length;
  const powerSavingCount = assessments.length
    ? assessments.filter((x) => x.power_monthly_saving > 0).length
    : latestInvoiceByMeter.filter((i) => invoicePowerSaving(i).amount > 0)
        .length;
  const reactiveSavingCount = assessments.length
    ? assessments.filter((x) => x.reactive_monthly_saving > 0).length
    : latestInvoiceByMeter.filter((i) => invoiceReactiveSaving(i) > 0).length;
  const aiAlertData = useMemo(() => {
    ...
    for (const i of latestRows) {
      ...
      const avgKwh = past.length
        ? past.reduce((s, h) => s + metrics(h).kwh, 0) / past.length
        : 0;
      const avgAmount = past.length
        ? past.reduce((s, h) => s + Number(h.total_amount || 0), 0) /
          past.length
        : 0;
      ...
      if (x.pf > 0 && x.pf < 0.95) {
        critical.push({
          kind: "pf",
          score: (0.95 - x.pf) * 100,
          label: m?.service_name || m?.meter_number || "Medidor",
          detail: `Cos φ ${x.pf.toFixed(3)} · revisar compensación reactiva`,
          invoice: i,
          meterId: i.meter_id,
        });
      }
      if (avgKwh > 0 && x.kwh > avgKwh * 1.3) {
        critical.push({
          kind: "consumption",
          score: (x.kwh / avgKwh - 1) * 100,
          label: m?.service_name || m?.meter_number || "Medidor",
          detail: `Consumo +${((x.kwh / avgKwh - 1) * 100).toFixed(0)}% vs promedio 6 meses`,
          invoice: i,
          meterId: i.meter_id,
        });
      }
      if (avgAmount > 0 && currentAmount > avgAmount * 1.35) {
        critical.push({
          kind: "amount",
          score: (currentAmount / avgAmount - 1) * 100,
          label: m?.service_name || m?.meter_number || "Medidor",
          detail: `Importe +${((currentAmount / avgAmount - 1) * 100).toFixed(0)}% vs promedio`,
          invoice: i,
          meterId: i.meter_id,
        });
      }
      ...
      if (x.excess > 0) {
        ...
        opportunities.push({
          ...
          value: monthly,
          label: m?.service_name || m?.meter_number || "Medidor",
          detail: `${number.format(x.excess)} kW sobrantes · ${money.format(monthly)}/mes`,
          invoice: i,
          meterId: i.meter_id,
        });
      }
      ...
      if (reactive > 0) {
        opportunities.push({
          ...
          value: reactive,
          label: m?.service_name || m?.meter_number || "Medidor",
          detail: `Penalización reactiva evitable · ${money.format(reactive)}/mes`,
          invoice: i,
          meterId: i.meter_id,
        });
      }
      const tariff = tariffSavings.find(
        (t) =>
          t.meter_id === i.meter_id &&
          String(t.billing_period).slice(0, 7) === latest,
      );
      if (Number(tariff?.monthly_saving_with_vat || 0) > 0) {
        opportunities.push({
          kind: "tariff",
          value: Number(tariff?.monthly_saving_with_vat || 0),
          label: m?.service_name || m?.meter_number || "Medidor",
          detail: `Cambio tarifario · ${money.format(Number(tariff?.monthly_saving_with_vat || 0))}/mes`,
          invoice: i,
          meterId: i.meter_id,
        });
      }
      ...
      if (prev && prevM) {
        const consumptionDelta = prevM.kwh
          ? ((x.kwh - prevM.kwh) / prevM.kwh) * 100
          : 0;
        const demandDelta = prevM.demand
          ? ((x.demand - prevM.demand) / prevM.demand) * 100
          : 0;
        const amountDelta = Number(prev.total_amount || 0)
          ? ((currentAmount - Number(prev.total_amount || 0)) /
              Number(prev.total_amount || 0)) *
            100
          : 0;
        if (
          Math.abs(consumptionDelta) >= 20 ||
          Math.abs(demandDelta) >= 20 ||
          Math.abs(amountDelta) >= 20
        ) {
          const bits = [
            Math.abs(consumptionDelta) >= 20
              ? `consumo ${consumptionDelta >= 0 ? "+" : ""}${consumptionDelta.toFixed(0)}%`
              : "",
            Math.abs(demandDelta) >= 20
              ? `demanda ${demandDelta >= 0 ? "+" : ""}${demandDelta.toFixed(0)}%`
              : "",
            Math.abs(amountDelta) >= 20
              ? `importe ${amountDelta >= 0 ? "+" : ""}${amountDelta.toFixed(0)}%`
              : "",
          ].filter(Boolean);
          changes.push({
            label: m?.service_name || m?.meter_number || "Medidor",
            detail: bits.join(" · "),
            positive: consumptionDelta < 0 && amountDelta < 0,
            invoice: i,
            meterId: i.meter_id,
          });
        }
    ...
    for (const m of missingPeriodMeters) {
      critical.push({
        ...
        score: 60,
        label: m.service_name || m.meter_number || "Medidor",
        detail: `Factura faltante en ${controlPeriod || latest}`,
        invoice: null,
        meterId: m.id,
      });
    }
  ...
  const dashboardPeriodLabel = dashboardPeriod
    ? new Date(
        Number(dashboardPeriod.slice(0, 4)),
        Number(dashboardPeriod.slice(5, 7)) - 1,
        1,
  ...
  useEffect(() => {
    ...
    async function loadAdvancedTariffSummary() {
      ...
      try {
        const result = await api<AdvancedTariffSummary>(
          `/api/organizations/${orgId}/tariff-saving-summary?period=${dashboardPeriod}`,
          session,
        );
  ...
  const dashboardInvoices = invoices.filter(
    (i) => invoiceMonth(i) === dashboardPeriod,
  );
  const dashboardPresentIds = new Set(dashboardInvoices.map((i) => i.meter_id));
  const dashboardReceived = [...dashboardPresentIds].filter((id) =>
    activeMeters.some((m) => m.id === id),
  ).length;
  ...
  const dashboardAdvancedRows =
    advancedTariffSummary?.billing_period === dashboardPeriod
      ? advancedTariffSummary.meters.filter(
          (x) => x.available && Number(x.monthly_saving || 0) > 0,
        )
      : [];
  ...
  const markerData = useMemo(() => {
    const counters = { west: 0, center: 0, east: 0 };
    return meters.map((m) => {
      const text =
        `${m.service_name || ""} ${m.sites?.name || ""} ${m.sites?.address || ""}`.toLowerCase();
      const zone = text.includes("oeste")
        ? "west"
        : text.includes("este")
          ? "east"
          : text.match(
                /centro|municipalidad|san martin|belgrano|plaza|radio|biblioteca|deportiva|social/,
              )
            ? "center"
            : text.match(/costa|pozo|bomba|agua|cloac|planta|vivero/)
              ? hashText(text) % 2
                ? "west"
                : "east"
              : (["west", "center", "east"][hashText(text) % 3] as
                  "west" | "center" | "east");
      const index = counters[zone]++,
        bounds =
          zone === "west" ? [9, 34] : zone === "center" ? [37, 62] : [65, 91],
      ...
      return {
        ...
        x:
          ...
          4 +
          (col * (width - 8)) / 4 +
          ((hashText(m.id) % 3) - 1) * 0.8,
        y: 14 + ((row * 11) % 70) + ((hashText(m.meter_number) % 3) - 1) * 0.7,
      };
    });
  }, [meters]);
          ...
          <>
            <section className="panel dashboard-ai-ask">
              <div className="dashboard-ai-copy">
                <span>
                  ✦ INTELIGENCIA ENERGÉTICA ·{" "}
                  {dashboardPeriodLabel.toUpperCase()}
                </span>
                <h2>¿Qué querés saber del período?</h2>
                <p>
                  Preguntale a la IA sobre consumo, facturas faltantes,
                  potencia, cos φ y oportunidades de ahorro del mes.
                </p>
              </div>
        ...
        {tab === "invoices" && (
          <>
            <div className="invoice-subtabs">
              <button
                className={invoiceSubTab === "received" ? "active" : ""}
              ...
              <button
                className={invoiceSubTab === "missing" ? "active missing" : ""}
                onClick={() => setInvoiceSubTab("missing")}
              >
                <span>Sin facturación reciente</span>
                <b>{lifecycleMeters.length}</b>
              </button>
              <button
                className={invoiceSubTab === "publicLighting" ? "active" : ""}
                onClick={() => setInvoiceSubTab("publicLighting")}
              >
              ...
              <>
                <section className="month-control">
                  <div>
                    ...
                    <strong>{controlPeriod || "Sin período"}</strong>
                  </div>
                  <div>
                    <span>Facturas esperadas</span>
                    <strong>{activeMeters.length}</strong>
                  </div>
                  <div className="received">
                    <span>Facturas recibidas</span>
                    <strong>
                      {
                        [...presentMeterIds].filter((id) =>
                          activeMeters.some((m) => m.id === id),
                        ).length
                      }
                    </strong>
                  ...
                  <div
                    className={
                      missingPeriodMeters.length ? "missing" : "complete"
                    }
                  >
                    <span>Faltantes de agosto</span>
                ...
                {missingPeriodMeters.length > 0 && (
                  <section className="panel missing-invoice-panel">
                    <Title
                      ...
                      sub="Estos medidores no aparecen en el archivo del período seleccionado"
                    />
                    <div className="missing-meter-grid">
                      {missingPeriodMeters.map((m) => (
                        <article key={m.id}>
                          <i>!</i>
                          <div>
                ...
                <section className="panel">
                  <Title
                    title="Administrador mensual y anual"
                    sub={`${filteredInvoices.length} de ${meters.length} facturas recibidas para ${controlPeriod}`}
                  />
                  <div className="filters">
                      ...
                      >
                        {Array.from({ length: 12 }, (_, i) =>
                          ...
                          <option key={m} value={m}>
                            {new Date(2026, Number(m) - 1, 1).toLocaleString(
                              "es-AR",
                              { month: "long" },
                            )}
                          </option>
                    ...
                    <label className="search-filter">
                      ...
                      <input
                        ...
                        placeholder="Medidor, suministro, servicio o factura"
                      />
                    </label>
                ...
                {selectedInvoice && (
                  <InvoiceAnalysisPanel
                    ...
                    changeControls={changeControls}
                    onChangeControls={() => loadChangeControls(orgId)}
            ...
            {invoiceSubTab === "missing" && (
              <div className="invoice-missing-subpage">
                <section className="panel invoice-missing-summary">
                  <div>
                    ...
                    <p>
                      Acá se muestran los medidores que llevan varios meses sin
                      facturación y requieren definir si continúan activos o
                ...
                <section className="panel invoice-missing-list-panel">
                  ...
                  <div className="invoice-missing-list-body">
                    {lifecycleMeters.map((m) => {
                      ...
                      return (
                        <div
                          className={`invoice-missing-list-row ${m.status === "removed" ? "removed" : "watch"}`}
                          key={m.id}
                        >
                          <div>
                            <span
                              className={`invoice-missing-state ${m.status || "inactive"}`}
                            >
                              {m.status === "removed"
                                ? "BAJA CONFIRMADA"
                                : "POSIBLE BAJA"}
                          ...
                          <div>
                            ...
                            <small>
                              {m.sites?.address || "Sin dirección registrada"}
                            </small>
                          </div>
                          <div>
                            <b>{m.supply_number || "S/D"}</b>
                            <small>
                              {m.current_tariff_code || "Sin tarifa"}
                            </small>
                          </div>
                          <div>
                          ...
                          <div className="invoice-missing-list-actions">
                            {m.status !== "removed" && (
                              <button
                                ...
                                onClick={() =>
                                  updateMeterStatus(m.id, "removed")
                                }
                              >
                                Confirmar baja
                              </button>
                            ...
                            <button
                              className="ok-btn"
                              onClick={() => updateMeterStatus(m.id, "active")}
                            >
                              Continúa activo
                            </button>
                          </div>
                    ...
                    {!lifecycleMeters.length && (
                      <div className="invoice-missing-list-empty">
                        No hay medidores sin facturación reciente.
                      </div>
                    )}
                  </div>
                </section>
        ...
        {tab === "framing" && (
          <>
            <EpenOptimizationPanel
              session={session}
              organizationId={orgId || ""}
              onOpenMeter={openMeterById}
            />
            <div className="savings-summary">
              <article>
                <span>Potencia contratada</span>
                <b>{money.format(powerAnnual)}</b>
                <small>ahorro anual</small>
              </article>
              <article>
                <span>Factor de potencia</span>
                <b>{money.format(reactiveAnnual)}</b>
                <small>recargos evitables</small>
              </article>
              <article>
                <span>Cambio tarifario</span>
              ...
              <article className="total">
                ...
                <small>{money.format(tariffAnnual / 12)} por mes</small>
              </article>
            </div>
            <div className="framing-meta">
              <span>{assessments.length} suministros analizados</span>
              <span>{reviewCount} requieren revisión</span>
              <span>
                {assessments.filter((x) => x.status === "correct").length}{" "}
                correctamente encuadrados
              </span>
            </div>
            <TariffSavingsTable rows={tariffSavings} />
            <section className="panel">
              <Title
                title="Diagnóstico y ahorros separados"
                sub="Potencia · Energía reactiva · Categoría tarifaria"
              />
              <div className="framing-tabs">
                <button
                  className={framingFilter === "all" ? "active" : ""}
                  onClick={() => setFramingFilter("all")}
                >
                  Todos
                </button>
                <button
                  className={
                    framingFilter === "change_candidate" ? "active" : ""
                  }
                  onClick={() => setFramingFilter("change_candidate")}
                >
                  Posible cambio
                </button>
                <button
                  className={framingFilter === "power_review" ? "active" : ""}
                  onClick={() => setFramingFilter("power_review")}
                >
                  Revisar potencia
                </button>
                <button
                  className={framingFilter === "provisional" ? "active" : ""}
                  onClick={() => setFramingFilter("provisional")}
                >
                  Provisorios
                </button>
                <button
                  className={framingFilter === "correct" ? "active" : ""}
                  onClick={() => setFramingFilter("correct")}
                >
                  Correctos
                </button>
              </div>
          ...
          <div className="ai-module">
            <section className="panel ai-hero">
              <div>
                <span className="ai-kicker">
                  ASISTENTE DE GESTIÓN ENERGÉTICA
                </span>
                <h2>IA para analizar la base municipal</h2>
                <p>
                  Consultá facturas, consumo, potencia, factor de potencia,
                  faltantes y oportunidades de ahorro.
                </p>
              </div>
            ...
            <section className="panel ai-chat">
              <div className="ai-chat-head">
                <div>
                  <h2>Preguntale a la base</h2>
                  <p>Conectado al backend, Supabase y OpenAI.</p>
                </div>
              </div>
              ...
              <div className="ai-answer">
                ...
                <div className="ai-answer-content">
                  <b>Asistente energético</b>
                  <div className="ai-rich-response">
                    {aiBusy ? (
                      <p className="ai-rich-paragraph">
                        Analizando Supabase con OpenAI…
                      </p>
                    ) : (
                      renderAiRichText(aiAnswer, openAiReference)
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="ai-input-row">
                <input
                  ...
                  placeholder="Ej.: ¿Qué 5 acciones concretas debería hacer primero para ahorrar este mes?"
                />
                <button onClick={() => runAiQuery()} disabled={aiBusy}>
                  {aiBusy ? "Analizando…" : "Consultar"}
                </button>
              </div>
            </section>
            ...
            <div className="ai-alert-grid">
              <article>
                <span>Faltantes de agosto</span>
                <b>{missingPeriodMeters.length}</b>
                <small>{controlPeriod || periods[0] || "Sin período"}</small>
              </article>
              <article>
                <span>Cos φ bajo</span>
                <b>
                  {
                    latestInvoiceByMeter.filter((i) => {
                      const p = metrics(i).pf;
                      return p > 0 && p < 0.95;
                    }).length
                  }
              ...
              <article>
                <span>Potencia sobrante</span>
                <b>
                  {
                    latestInvoiceByMeter.filter((i) => metrics(i).excess > 0)
                      .length
                  }
                </b>
                <small>medidores detectados</small>
              </article>
              <article className="green">
                <span>Ahorro anual potencial</span>
                <b>{money.format(dashboardTotalMonthly * 12)}</b>
                <small>
                  {dashboardPeriodLabel} · mensual{" "}
                  {money.format(dashboardTotalMonthly)}
                </small>
              </article>
            </div>
            ...
            <div className="ai-smart-sections">
              <section className="panel ai-smart-card critical">
                <div className="ai-smart-head">
                  <div>
                    <span>ALERTAS CRÍTICAS</span>
                    <h3>Qué revisar ahora</h3>
                  </div>
                  <b>{aiAlertData.criticalCount}</b>
                </div>
                <div className="ai-smart-list">
                  {aiAlertData.critical.length ? (
                    aiAlertData.critical.map((a, index) => (
                      <button
                        key={`${a.kind}-${index}`}
                        className="ai-alert-clickable"
                        title="Abrir análisis del medidor"
                        onClick={() =>
                          a.invoice
                            ? openMeter(a.invoice)
                            : openMeterById(a.meterId)
                        }
                      >
                        <i>!</i>
                        <div>
                    ...
                    <div className="ai-smart-empty">
                      No hay alertas críticas para el último período.
                    </div>
                  )}
                </div>
              </section>
              ...
              <section className="panel ai-smart-card opportunities">
                <div className="ai-smart-head">
                  <div>
                    ...
                    <h3>Dónde hay más ahorro</h3>
                  </div>
                  <b>{aiAlertData.opportunityCount}</b>
                </div>
                <div className="ai-smart-list">
                  {aiAlertData.opportunities.length ? (
                    aiAlertData.opportunities.map((a, index) => (
                      <button
                        key={`${a.kind}-${index}`}
                        className="ai-alert-clickable"
                        title="Abrir análisis del medidor"
                      ...
                      >
                        ...
                        <em>{money.format(a.value)}</em>
                      </button>
                    ))
                  ) : (
                    <div className="ai-smart-empty">
                      No hay oportunidades valorizadas en el último período.
                    </div>
                  )}
                </div>
              </section>
              ...
              <section className="panel ai-smart-card changes">
                <div className="ai-smart-head">
                  <div>
                    ...
                    <h3>Variaciones relevantes</h3>
                  </div>
                  <b>{aiAlertData.changes.length}</b>
                </div>
                <div className="ai-smart-list">
                  {aiAlertData.changes.length ? (
                    aiAlertData.changes.map((a, index) => (
                      <button
                        key={index}
                        className="ai-alert-clickable"
                        title="Abrir análisis del medidor"
                      ...
                      >
                        ...
                        <em className={a.positive ? "positive" : ""}>
                          {a.positive ? "Mejora" : "Cambio"}
                        </em>
                      </button>
                    ))
                  ) : (
                    <div className="ai-smart-empty">
                      No se detectaron cambios superiores al 20% contra el mes
                      anterior.
                    </div>
                  )}
                </div>
...
function Title({
  ...
  return (
    <div className="panel-title">
      ...
      {action && <button onClick={action}>Ver todas →</button>}
    </div>
  );
}
...
function invoiceReactiveSaving(i: Invoice) {
  return (
    (i.invoice_lines || [])
...
function minimumContractedKw(tariff?: string) {
  const code = String(tariff || "")
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, "");
  return code.startsWith("T3") ? 50 : code.startsWith("T2") ? 10 : 0;
}
function invoicePowerSaving(i: Invoice) {
  const lines = i.invoice_lines || [],
    powerLines = lines.filter(
      (x) => x.concept_code === "DEM" || x.concept_code === "DEP",
    ),
    billed = Math.max(0, ...powerLines.map((x) => Number(x.quantity || 0))),
    contracted = Number(
      i.contracted_kw_peak || i.meters?.contracted_kw_peak || billed,
    ),
    demand = Math.max(
      0,
      ...(i.invoice_measurements || []).map((m) =>
    ...
    units = Math.max(0, contracted - proposed),
    rate = Math.max(0, ...powerLines.map((x) => Number(x.unit_price || 0))),
    amount = units * rate * 1.3;
...
function metrics(i: Invoice) {
  ...
  const contracted = Number(
    i.contracted_kw_peak ||
      i.meters?.contracted_kw_peak ||
      Math.max(
        0,
        ...(i.invoice_lines || [])
          .filter((x) => x.concept_code === "DEM" || x.concept_code === "DEP")
          .map((x) => Number(x.quantity || 0)),
      ),
  );
  const pfs = ms
  ...
  const penalized =
    ...
    surcharge > 0 ||
    cosCharge > 0;
  const minimumKw = minimumContractedKw(
    i.current_tariff_code || i.meters?.current_tariff_code,
  );
  ...
  return {
    ...
    demand,
    contracted,
    excess: Math.max(0, contracted - proposed),
    minimumKw,
    proposed,
    pf,
    surcharge,
...
function InvoiceTable({
  ...
  advancedTariffSummary: AdvancedTariffSummary | null;
  ...
  const [sort, setSort] = useState<
    ...
    order = (field: "consumption" | "demand" | "pf" | "tariff") => {
      if (sort === field) setDirection((x) => (x === "asc" ? "desc" : "asc"));
      else {
        setSort(field);
        setDirection(
          field === "consumption" || field === "demand" ? "desc" : "asc",
        );
      }
    },
    orderPower = () => {
      setSort((x) => (x === "contracted" ? "excess" : "contracted"));
      setDirection("desc");
    },
    value = (i: Invoice) =>
      sort === "consumption"
    ...
    sorted = [...invoices].sort((a, b) => {
      ...
      if (sort === "pf") {
        const ap = metrics(a).pf,
          bp = metrics(b).pf,
          ar = ap <= 0 ? 2 : ap < 0.95 ? 0 : 1,
          br = bp <= 0 ? 2 : bp < 0.95 ? 0 : 1;
      ...
      const av = value(a),
        ...
        result =
          typeof av === "string"
            ? av.localeCompare(String(bv), "es", { numeric: true })
            : Number(av) - Number(bv);
      return direction === "asc" ? result : -result;
    }),
    head = (
      field: "consumption" | "demand" | "pf" | "tariff",
      label: string,
    ) => (
      <button className="sort-head" onClick={() => order(field)}>
        {label}
        <i>{sort === field ? (direction === "asc" ? "▲" : "▼") : "↕"}</i>
      </button>
    ),
    powerHead = (
      <button className="sort-head power-sort" onClick={orderPower}>
        <span>
          Contratada / sobrante
          <small>
            {sort === "contracted"
              ? "Orden: potencia contratada"
              : sort === "excess"
                ? "Orden: mayor sobrante"
                : "Tocar para ordenar"}
          </small>
        </span>
        <i>
          {sort === "contracted" ? "kW ▼" : sort === "excess" ? "SOB ▼" : "↕"}
        </i>
      </button>
    );
  return (
    <div className="table invoice-admin-scroll">
      <table className="invoice-admin">
        <thead>
          <tr>
            <th>Medidor / ID</th>
            <th>Servicio</th>
            <th>Mes facturado</th>
            <th>{head("consumption", "Consumo")}</th>
            <th>{head("demand", "Potencia registrada")}</th>
            <th>{powerHead}</th>
            <th>
              <button
                className={`sort-head pf-sort-head ${sort === "pf" ? "active" : ""}`}
                onClick={() => order("pf")}
              >
                <span>
                  Factor potencia
                  <small>
                    {sort === "pf"
                      ? direction === "asc"
                        ? "Malos primero · peor → mejor"
                        : "Malos primero · mejor → peor"
                      : "Tocar: FP < 0,95 primero"}
                  </small>
                </span>
                <i>
                  {sort === "pf" ? (direction === "asc" ? "⚠ ▲" : "⚠ ▼") : "↕"}
                </i>
              </button>
            </th>
            <th>{head("tariff", "Tarifa")}</th>
            <th>Medidas de ahorro</th>
        ...
        <tbody>
          {sorted.map((i) => {
            const x = metrics(i),
              bad = x.pf > 0 && x.pf < 0.95,
              period = (i.billing_period || i.period_start).slice(0, 7),
              tariffResult = tariffSavings.find(
                (t) =>
                  t.meter_id === i.meter_id &&
                  String(t.billing_period).slice(0, 7) === period,
              ),
              advanced = epenOptimization.find(
                (e) => e.meter_id === i.meter_id,
              ),
              candidate = assessments.find((a) => a.meter_id === i.meter_id),
              assessment =
                candidate &&
              ...
              advancedTariffRow =
                advancedTariffSummary?.billing_period === period
                  ? advancedTariffSummary.meters.find(
                      (t) =>
                        (t.meter_id === i.meter_id ||
                          String((t as any).meter_number || "") ===
                            String(i.meters?.meter_number || "") ||
                          String((t as any).supply_number || "") ===
                            String(i.meters?.supply_number || "")) &&
                        String(t.billing_period).slice(0, 7) === period &&
                        t.available,
                    )
                  : undefined,
              ...
              controlPhase = appliedControls.length
                ? "after"
                : meterControls.length
                  ? "before"
                  : "none";
            return (
              <tr
                key={i.id}
                className={`selectable${bad ? " pf-problem-row" : ""}${controlPhase !== "none" ? ` control-${controlPhase}` : ""}`}
                onClick={() => onSelect?.(i)}
              >
                <td>
                ...
                <td>
                  <b>
                    {i.meters?.service_name ||
                      i.meters?.sites?.name ||
                      "Servicio sin nombre"}
                  </b>
                  <small>Suministro {i.meters?.supply_number || "S/D"}</small>
                </td>
                <td>
                  <b>{(i.billing_period || i.period_start).slice(0, 7)}</b>
                  ...
                  {appliedControls.length > 0 && (
                    <small className="invoice-control-types">
                      {[
                        ...new Set(
                          appliedControls.map((row) =>
                ...
                <td>
                  {x.pf ? (
                    <span className={`status-pill ${bad ? "bad" : "good"}`}>
                      {x.pf.toFixed(3)} {bad ? "Bajo" : "Correcto"}
                    </span>
                  ) : x.penalized ? (
                    <span className="status-pill bad">Penalizado · FP S/D</span>
                  ) : (
                    <span className="status-pill neutral">No detectado</span>
                  )}{" "}
                  {x.surcharge > 0 && (
                    <small className="danger">Recargo {x.surcharge}%</small>
                  )}
                </td>
                <td>
                  <div className="tariff-advanced-cell">
                    <div>
                      <span className="tag low">
                        {i.current_tariff_code || "S/D"}
                      </span>
                      <small>
                        {tariffResult &&
                        tariffResult.current_tariff !==
                          tariffResult.recommended_tariff ? (
                          <>
                            {tariffResult.current_tariff} →{" "}
                            {tariffResult.recommended_tariff}
                          </>
                        ) : (
                          i.voltage_level || i.meters?.voltage_level
                        )}
                      </small>
                    </div>
                    {advanced && (
                      <div className="tariff-advanced-badges">
                        {advanced.t3.status === "candidate" && (
                          <span className="t3-dual">T3 · 2 POTENCIAS</span>
                        )}
                        {advanced.t4.status === "candidate" && (
                          <span className="t4-candidate">APTO T4</span>
                        )}
                        {["strong", "candidate", "preliminary"].includes(
                          advanced.mt.status,
                        ) && <span className="mt-candidate">BT → MT</span>}
                      </div>
                    )}
                  </div>
                </td>
                <td>
                  ...
                  >
                    {measures.length ? (
                      ...
                      <small>Sin ahorro detectado</small>
                    )}
                  </div>
                </td>
                <td>
                ...
                <td>
                  <strong className="save">
                    {money.format(Number(i.total_amount || 0))}
                  </strong>
          ...
          {pendingMeters.length > 0 && (
            <>
              <tr className="pending-section-row">
                <td colSpan={11}>
                  <div className="pending-section-content">
                    <b>Facturas pendientes del período</b>
                    <span>{pendingMeters.length} filas pendientes</span>
                  </div>
                </td>
              </tr>
              {pendingMeters.map((m) => {
                const possibleRemoval = m.status === "inactive";
                return (
                  <tr
                    key={`pending-${m.id}`}
                    className={`pending-invoice-row selectable${possibleRemoval ? " possible-removal" : ""}`}
                    onClick={() => onSelectMeter?.(m.id)}
                    title="Abrir análisis histórico del medidor"
                  >
                    <td>
                      <b>Medidor {m.meter_number || "S/D"}</b>
                      <small>{m.tracking_code || "Sin ID"}</small>
                    ...
                    <td>
                      ...
                      <small>Suministro {m.supply_number || "S/D"}</small>
                    </td>
                    <td>
                      <b>{period}</b>
                      <small>Factura faltante</small>
                    </td>
                    <td colSpan={7}>
                      <div className="pending-placeholder">
                        {possibleRemoval
                          ? "Sin facturación reciente: posible baja, aún debe solicitarse la factura"
                          : `No se cargó una factura para este medidor en ${period}`}
                      </div>
                    </td>
                    <td>
                      <span className="pending-badge">
                        {possibleRemoval
                          ? "POSIBLE BAJA · PENDIENTE"
                          : "PENDIENTE"}
          ...
          {!invoices.length && !pendingMeters.length && (
            <tr>
              <td colSpan={11}>
                <div className="empty">
...
function TariffTable({
  ...
  const labels = {
      correct: "Correcto",
      power_review: "Revisar potencia",
      change_candidate: "Posible cambio",
      provisional: "Provisorio",
    },
    [sort, setSort] = useState<"power" | "pf" | "tariff" | "saving" | null>(
      null,
    ),
    [direction, setDirection] = useState<"asc" | "desc">("asc");
  const order = (field: "power" | "pf" | "tariff" | "saving") => {
      if (sort === field) setDirection((x) => (x === "asc" ? "desc" : "asc"));
      else {
        setSort(field);
        setDirection("asc");
    ...
    head = (field: "power" | "pf" | "tariff" | "saving", label: string) => (
      <button className="sort-head" onClick={() => order(field)}>
        {label}
        <i>{sort === field ? (direction === "asc" ? "▲" : "▼") : "↕"}</i>
      </button>
  ...
  return (
    <div className="table">
      <table className="tariff-table split-savings">
        <thead>
          <tr>
            <th>Medidor / servicio</th>
            <th>{head("power", "Potencia contratada")}</th>
            <th>{head("pf", "Factor de potencia")}</th>
            <th>{head("tariff", "Tarifa y posible arreglo")}</th>
            <th>Diagnóstico</th>
            <th>{head("saving", "Total propuesto")}</th>
          </tr>
        </thead>
        <tbody>
          {sorted.map((x) => (
            <tr
              ...
              onClick={() => onMeter(x.meter_id)}
            >
              <td>
                <b>{x.meter.service_name || "Servicio sin nombre"}</b>
                <small>
                  Medidor {x.meter.meter_number} · {x.meter.tracking_code}
                </small>
                <small>
                  {x.periods_analyzed} período
                  {x.periods_analyzed === 1 ? "" : "s"} · Confianza{" "}
                  {x.confidence}%
                </small>
              </td>
              <td>
              ...
              <td>
                <b>
                  {x.power_factor ? x.power_factor.toFixed(3) : "No detectado"}
                </b>
                <small>
                  {x.power_factor && x.power_factor < 0.95
              ...
              <td>
                <div>
                  <span className="tariff-code">
                    {x.current_tariff || "S/D"}
                  </span>
                  <b className="arrow">→</b>
                  <span
                    className={
                      x.current_tariff === x.recommended_tariff
                        ? "tariff-code same"
                        : "tariff-code recommended"
                    }
                  >
                    {x.recommended_tariff}
                  </span>
                </div>
                <small>
                  {x.official_schedule
                    ? `Cuadro EPEN ${x.official_schedule.resolution_number} · vigencia ${x.official_schedule.valid_from} a ${x.official_schedule.valid_to}`
                ...
                {x.tariff_simulation_available &&
                  ...
                  x.tariff_recommended_simulated != null && (
                    <small>
                      Costo vigente: {money.format(x.tariff_current_simulated)}{" "}
                      → {money.format(x.tariff_recommended_simulated)}
                    </small>
                  )}
              </td>
              <td>
                <span className={`assessment ${x.status}`}>
                  {labels[x.status]}
                </span>
                <p className="assessment-reason">{x.reasons.join(" · ")}</p>
              </td>
              <td>
...
function TariffSavingsTable({ rows }: { rows: TariffSaving[] }) {
  if (!rows.length) return null;
  return (
    <section className="panel tariff-savings-panel">
      <Title
        title="Cambios tarifarios valorizados"
        sub={`${rows.filter((x) => Number(x.monthly_saving_with_vat) > 0).length} con ahorro positivo · ${rows.length} candidatos`}
      />
      <div className="table">
        <table>
          <thead>
            <tr>
              ...
              <th>Potencia usada</th>
              <th>Costo tarifa actual</th>
              <th>Costo recomendado</th>
          ...
          <tbody>
            {rows.map((x) => (
              <tr key={x.meter_id}>
                <td>
                  <b>{x.service_name || "Servicio sin nombre"}</b>
                  <small>
                    Medidor {x.meter_number} · Suministro{" "}
                    {x.supply_number || "S/D"}
                  </small>
                </td>
                <td>{String(x.billing_period).slice(0, 7)}</td>
                <td>
                  <span className="tariff-code">{x.current_tariff}</span>
                  <b className="arrow">→</b>
                  <span className="tariff-code recommended">
                    {x.recommended_tariff}
                  </span>
                </td>
                <td>
                  <b>{number.format(Number(x.used_kw || 0))} kW</b>
                </td>
                <td>{money.format(Number(x.current_cost_with_vat || 0))}</td>
                <td>
                  {money.format(Number(x.recommended_cost_with_vat || 0))}
                </td>
                <td>
                  <strong
                    className={
                      Number(x.monthly_saving_with_vat) > 0
                        ? "save"
                        : "muted-saving"
                    }
                  >
                    {money.format(Number(x.monthly_saving_with_vat || 0))}
                  </strong>
                </td>
                <td>
                  <strong
                    className={
                      Number(x.annual_saving_with_vat) > 0
                        ? "save"
                        : "muted-saving"
                    }
                  >
                    {money.format(Number(x.annual_saving_with_vat || 0))}
                  </strong>
                </td>
              </tr>
            ))}
...
function monthsBetween(from?: string, to?: string) {
  if (!from || !to) return 0;
  const [a, b] = from.slice(0, 7).split("-").map(Number),
    [c, d] = to.slice(0, 7).split("-").map(Number);
  return Math.max(0, (c - a) * 12 + d - b);
}
function MeterLifecyclePanel({
  meters,
  latestPeriod,
  onStatus,
}: {
  meters: Meter[];
  latestPeriod: string;
  onStatus: (id: string, status: "active" | "inactive" | "removed") => void;
}) {
  if (!meters.length) return null;
  return (
    <section className="panel lifecycle-panel">
      <Title
        title="Sin facturación recienteción reciente"
        sub="Se controlan por separado y no se cuentan como facturas faltantes"
      />
      <div className="lifecycle-grid">
        {meters.map((m) => (
          <article
            key={m.id}
            className={m.status === "removed" ? "removed" : "watch"}
          >
            <div className="lifecycle-icon">
              {m.status === "removed" ? "×" : "!"}
            </div>
            <div className="lifecycle-info">
              <span className={`lifecycle-status ${m.status}`}>
                {m.status === "removed" ? "BAJA CONFIRMADA" : "POSIBLE BAJA"}
              </span>
              <b>{m.service_name || m.sites?.name || "Servicio sin nombre"}</b>
              <small>
                Medidor {m.meter_number || "S/D"} · Suministro{" "}
                {m.supply_number || "S/D"}
              </small>
              <dl>
                <div>
                  <dt>Última factura</dt>
                  <dd>{m.last_seen_period?.slice(0, 7) || "S/D"}</dd>
                </div>
                <div>
                  <dt>Meses sin factura</dt>
                  <dd>{monthsBetween(m.last_seen_period, latestPeriod)}</dd>
                </div>
              </dl>
            </div>
...
function MissingInvoiceTable({
  ...
  return (
    <div className="missing-table">
      <div className="missing-table-title">
        <b>Facturas pendientes del período</b>
        <span>{meters.length} filas pendientes</span>
      </div>
      <table className="invoice-admin">
        <tbody>
          {meters.map((m) => {
            ...
            return (
              <tr
                key={m.id}
                className={`missing-invoice-row${possibleRemoval ? " possible-removal" : ""}`}
                style={possibleRemoval ? { background: "#fff4c2" } : undefined}
              >
                <td>
                  <b>Medidor {m.meter_number || "S/D"}</b>
                  <small>{m.tracking_code || "Sin ID"}</small>
                ...
                <td>
                  <b>
                    {m.service_name || m.sites?.name || "Servicio sin nombre"}
                  </b>
                  <small>Suministro {m.supply_number || "S/D"}</small>
                </td>
                <td>
                ...
                <td>
                  <span className="missing-badge">
                    {possibleRemoval ? "POSIBLE BAJA · PENDIENTE" : "PENDIENTE"}
                  </span>
                </td>
              </tr>
            );
...
function Saving({ value }: { value: number }) {
  return (
    <div className={value > 0 ? "saving-chip positive" : "saving-chip"}>
...
function MeterDetail({
  ...
  const m = invoice.meters,
    x = metrics(invoice),
    sorted = [...history].sort((a, b) =>
      (b.billing_period || b.period_start).localeCompare(
        a.billing_period || a.period_start,
      ),
    );
  return (
    <div className="detail-backdrop" onClick={onClose}>
      <aside className="meter-detail" onClick={(e) => e.stopPropagation()}>
        <div className="detail-head">
          <div>
            <small>DETALLE DEL MEDIDOR</small>
            <h2>
              {m?.service_name || m?.sites?.name || "Servicio sin nombre"}
            </h2>
            <p>
              {m?.tracking_code} · Medidor {m?.meter_number}
            </p>
          </div>
        ...
        <div className="detail-kpis">
          ...
          <article className={x.excess > 0 ? "alert" : ""}>
            <span>Exceso de potencia</span>
            <b>{number.format(x.excess)} kW</b>
          </article>
          <article>
            <span>Factor de potencia</span>
            <b>{x.pf ? x.pf.toFixed(3) : "No detectado"}</b>
          </article>
        </div>
        <section className="detail-section">
          ...
          <dl>
            <div>
              <dt>ID seguimiento</dt>
              <dd>{m?.tracking_code || "S/D"}</dd>
            </div>
            <div>
              <dt>Medidor</dt>
              <dd>{m?.meter_number || "S/D"}</dd>
            </div>
            <div>
              <dt>Suministro / contrato</dt>
              <dd>
                {m?.supply_number || "S/D"} / {m?.contract_number || "S/D"}
              </dd>
            </div>
            <div>
              <dt>Código de servicio</dt>
              <dd>{m?.service_code || "S/D"}</dd>
            </div>
            <div>
              <dt>Nomenclatura catastral</dt>
              <dd>{m?.cadastral_number || "S/D"}</dd>
            </div>
            <div>
              <dt>Tensión / tarifa</dt>
              <dd>
                {invoice.voltage_level || m?.voltage_level || "S/D"} ·{" "}
                {invoice.current_tariff_code || "S/D"}
              </dd>
            </div>
          </dl>
        </section>
        <section className="detail-section">
          ...
          <dl>
            <div>
              <dt>Mes facturado</dt>
              <dd>
                {(invoice.billing_period || invoice.period_start).slice(0, 7)}
              </dd>
            </div>
            <div>
              <dt>Número de factura</dt>
              <dd>{invoice.invoice_number || "S/D"}</dd>
            </div>
            <div>
              <dt>Potencia contratada</dt>
              <dd>{number.format(x.contracted)} kW</dd>
            </div>
            <div>
              <dt>Importe</dt>
              <dd>{money.format(Number(invoice.total_amount || 0))}</dd>
            </div>
            <div>
              <dt>Vencimiento</dt>
              <dd>{invoice.due_date || "S/D"}</dd>
            </div>
            <div>
              <dt>Deuda anterior</dt>
              <dd>{money.format(Number(invoice.previous_debt_amount || 0))}</dd>
            </div>
          </dl>
        </section>
        <section className="detail-section">
          ...
          <div className="mini-history">
            {sorted.map((h) => {
              ...
              return (
                <button key={h.id}>
                  <span>
                    {(h.billing_period || h.period_start).slice(0, 7)}
                  </span>
                  <b>{number.format(z.kwh)} kWh</b>
                  <em>{number.format(z.demand)} kW</em>
                  <strong>{money.format(Number(h.total_amount || 0))}</strong>
                </button>
              );
            })}
          </div>
