...
type Meter={id:string;meter_number?:string;tracking_code?:string;service_name?:string;sites?:{name?:string}};
...
export function HistoricalAnalysis({invoices,meters,tariffSavings}:{invoices:Invoice[];meters:Meter[];tariffSavings:TariffSaving[]}){
  ...
  return <div className="historical-analysis">
    ...
    <section className="panel history-panel meter-history"><div className="history-head"><div><h2>Análisis por medidor</h2><p>Seleccioná un suministro para revisar su comportamiento mes a mes</p></div><label>Medidor<select value={meterId} onChange={event=>setSelectedMeter(event.target.value)}>{meters.map(row=><option value={row.id} key={row.id}>{row.meter_number||"S/D"} · {row.service_name||row.sites?.name||row.tracking_code}</option>)}</select></label></div>
