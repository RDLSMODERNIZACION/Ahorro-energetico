...
export function MeterLocationEditor({
  meterId,
  label,
  onClose,
}: {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setMessage(
            error instanceof Error ? error.message : "No se pudo consultar la ubicación",
          );
        }
      }
    })();
  ...
  useEffect(() => {
    ...
    loadLeaflet()
      ...
      .catch((error) =>
        setMessage(error instanceof Error ? error.message : "No se pudo cargar el mapa"),
      );
  ...
  function useMyLocation() {
    ...
    if (!navigator.geolocation) {
      setMessage("El navegador no permite obtener ubicación.");
      return;
    }
  ...
  async function save() {
    ...
    if (!Number.isFinite(a) || a < -90 || a > 90) {
      setMessage("Latitud inválida.");
      return;
    }
    if (!Number.isFinite(b) || b < -180 || b > 180) {
      setMessage("Longitud inválida.");
      return;
    }
    setBusy(true);
    setMessage("");
  ...
  return (
    <section className="invoice-analysis-panel meter-location-editor">
      ...
      <div className="meter-location-layout">
        <div className="meter-location-form">
          ...
          <div className="meter-location-help">
            ...
            <span>
              Tocá cualquier punto del mapa. Después podés arrastrar el marcador para ajustarlo
              con precisión.
            </span>
          </div>

          {original && (
            <div className="meter-location-current">
              <span>Última ubicación registrada</span>
              <b>
                {Number(original.latitude).toFixed(6)}, {Number(original.longitude).toFixed(6)}
              </b>
              <small>
                {original.valid_from
                  ? `Desde ${new Date(original.valid_from).toLocaleString("es-AR")}`
                  : "Ubicación vigente"}
              </small>
            </div>
          )}
            ...
            <div
              className={
                message.includes("correctamente")
                  ? "meter-location-message ok"
                  : "meter-location-message"
              }
            >
              {message}
            </div>
        ...
        <div className="meter-location-map-wrap">
          ...
          <div className="meter-location-map-note">
            Mapa OpenStreetMap · clic para posicionar · marcador arrastrable
          </div>
        </div>
      </div>
    </section>
