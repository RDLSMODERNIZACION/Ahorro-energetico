...
$backup=Join-Path $Root "backup_v11_$stamp"
...
# -------- Tipos --------
$text=$text.Replace(
  '  capacity_kw:number;'+[Environment]::NewLine+'};',
  '  capacity_kw:number;'+[Environment]::NewLine+'  available?:boolean;'+[Environment]::NewLine+'  reason?:string|null;'+[Environment]::NewLine+'};'
)

# -------- Cálculo de ahorro: usar el mismo valor avanzado que el gráfico --------
$old=@'
...
if(-not $text.Contains($old)){throw "No encontré el cálculo tariffSaving actual."}
$text=$text.Replace($old,$new)

# -------- Reemplazar SOLO TariffSavingTrend --------
...
# -------- Pasar available al gráfico avanzado --------
$oldAdvanced=@'
...
'@
if(-not $text.Contains($oldAdvanced)){throw "No encontré advancedRows."}
$text=$text.Replace($oldAdvanced,$newAdvanced)

# Legacy siempre disponible
$oldLegacy=@'
              current_tariff:x.current_tariff,
              recommended_tariff:x.recommended_tariff
            }));
...
'@
# sólo primera ocurrencia (legacy)
$idx=$text.IndexOf($oldLegacy,$text.IndexOf('const legacyRows='))
if($idx -ge 0){
  $text=$text.Substring(0,$idx)+$newLegacy+$text.Substring($idx+$oldLegacy.Length)
}

# -------- Texto del cuadro inferior --------
$oldBottom='<div><span>Encuadramiento tarifario</span><b>{money.format(tariffSaving)}</b><small>{tariffSaving>0?"Ahorro mensual simulado con IVA":"Sin ahorro tarifario valorizado"}</small></div>'
$newBottom='<div><span>Encuadramiento tarifario</span><b>{money.format(tariffSaving)}</b><small>{tariffSavingSource==="T4"?`${advancedTariffPoint?.current_tariff||"Actual"} → ${advancedTariffPoint?.recommended_tariff||"T4"} · simulación antes de impuestos`:tariffSavingSource==="legacy"?"Ahorro mensual simulado con IVA":advancedTariffPoint?.available===false?"Falta cuadro tarifario T4 para este período":"Sin ahorro tarifario valorizado"}</small></div>'
if(-not $text.Contains($oldBottom)){throw "No encontré Encuadramiento tarifario inferior."}
...
if($c -notmatch 'EPEN V11 COMPLETE HISTORY'){
Add-Content $css @'

/* EPEN V11 COMPLETE HISTORY */
...
Write-Host ""
Write-Host "OK - V11 aplicada." -ForegroundColor Green
Write-Host "Cambios:" -ForegroundColor Yellow
Write-Host "  - conserva todos los meses del histórico T3/T4"
Write-Host "  - meses sin tarifa T4 cargada aparecen en gris"
Write-Host "  - Encuadramiento tarifario inferior usa el MISMO ahorro que el gráfico"
Write-Host "  - Total mensual y Ahorro potencial también incluyen ese ahorro"
Write-Host "  - elimina duplicados mensuales/notas de crédito para elegibilidad T4"
Write-Host ""
Write-Host "IMPORTANTE: hay cambios de BACKEND. Debe desplegarse en Render."
